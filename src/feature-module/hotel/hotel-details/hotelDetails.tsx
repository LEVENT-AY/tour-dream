...
const REPLACEMENT_CHAR_PATTERN = /\uFFFD|√Ø¬ø¬Ω/;
const MOJIBAKE_PATTERN = /(?:√É.|√Ç|√¢‚Ç¨|√¢‚Ç¨‚Ñ¢|√¢‚Ç¨≈ì|√¢‚Ç¨\u009d|√¢‚Ç¨‚Äú|√¢‚Ç¨‚Äù)/;
...
const NEARBY_MARKERS = [
  /\bles\s+environs\b/i,
  /\brestaurants?\s+‡\s+proximitÈ\b/i,
  /\bcafÈs?\s+aux\s+alentours\b/i,
  /\bhÙtels?\s+‡\s+proximitÈ\b/i,
  /\blieux\s+‡\s+proximitÈ\b/i,
  /\bsites?\s+historiques\b/i,
  /\battractions?\s+‡\s+proximitÈ\b/i,
...
const PREFERRED_AMENITY_PATTERNS: Array<{ pattern: RegExp; score: number }> = [
  ...
  { pattern: /\bspa|bien-?√™tre|wellness|massage\b/i, score: 38 },
...
const SECTION_TITLE_PATTERNS: Array<{ pattern: RegExp; title: string }> = [
  { pattern: /\brestaurants?\s+‡\s+proximitÈ\b/i, title: 'Restaurants ‡ proximitÈ' },
  { pattern: /\bcafÈs?\s+aux\s+alentours\b/i, title: 'CafÈs aux alentours' },
  { pattern: /\bhÙtels?\s+‡\s+proximitÈ\b/i, title: 'HÙtels ‡ proximitÈ' },
  { pattern: /\blieux\s+‡\s+proximitÈ\b/i, title: 'Lieux ‡ proximitÈ' },
  { pattern: /\bsites?\s+historiques\b/i, title: 'Sites historiques' },
  { pattern: /\battractions?\s+‡\s+proximitÈ\b/i, title: 'Attractions ‡ proximitÈ' },
...
const buildDescriptionLead = (data: Record<string, any>, title: string) => {
  ...
  const regionLabel = region ? `sur l'√Æle de ${region.charAt(0).toUpperCase()}${region.slice(1)}` : "sur l'√Æle de Djerba";
  const cityLabel = normalizedCity ? `dans la r√©gion de ${normalizedCity}` : "dans la r√©gion de Midoun";
  return `Le ${leadTitle} est un h√¥tel ${Math.round(starRating)} √©toiles situ√© ${regionLabel}, ${cityLabel}.`;
};
  ...
  for (const candidate of descriptionCandidates) {
    ...
    const sanitized = trimAtNearbyMarkers(
      sanitizeRequestOnlyCopy(raw)
        .replace(/^(r√©servez d√®s maintenant|book now|instant booking|confirmed booking|pay now|guaranteed booking)[^.]*\.\s*/i, "")
        .replace(/^(votre s√©jour\b[^.]*\.\s*)/i, "")
        .trim(),
    );
    if (!sanitized || hasReplacementCharacter(sanitized)) continue;
    if (/^(votre s√©jour|votre sejour)/i.test(sanitized) || sanitized.length < 60) {
      const fallback = buildDescriptionLead(data, title);
      if (fallback) return fallback;
    }
    if (!/^(Le|La|Les|L['‚Äô]|Ce|Cette|Cet|L‚Äô|The|Hotel|H√¥tel)\b/.test(sanitized)) {
      const fallback = buildDescriptionLead(data, title);
      if (fallback) return fallback;
    }
    return sanitized;
...
const HotelDetails = () => {
  ...
  const visibleFaqItems = displayHotel
    ? displayHotel.faq.filter((item) => !/(\bprice\b|\bprix\b|\btarif\b|\bnuit√©e\b|\bnight\b)/i.test(item.question))
    : [];
