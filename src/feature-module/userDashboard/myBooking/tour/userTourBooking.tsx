...
import { fetchUserBookings } from '../../../../core/services/firebaseServices';
...
const UserTourBooking = () => {
  ...
  useEffect(() => {
    const getBookings = async () => {
      ...
      if (currentUser) {
        try {
          const dbBookings = await fetchUserBookings(currentUser.uid);
          const formatted = dbBookings.map((b: any, idx: number) => ({
            key: b.id || idx.toString(),
            id: `#${(b.id || "").substring(0, 7).toUpperCase()}`,
            tourimage: b.itemImage || "assets/img/tours/tours-20.jpg",
            tourname: b.itemTitle,
            tourtype: b.itemType.charAt(0).toUpperCase() + b.itemType.slice(1) + " Booking",
            travellers: "1 Adult",
            day: "4 Days",
            pricing: `$${b.price}`,
            date: b.bookingDate,
