...
const AdminServiceRequests: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    return requests.filter((r) => {
      const text = `${r.serviceTitle || ''} ${r.customerName || ''} ${r.customerEmail || ''} ${r.customerPhone || ''} ${r.serviceType || ''}`.toLowerCase();
                  ...
                  filtered.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td>
                        ...
                        <div className="fs-12 text-muted">{r.customerEmail || '-'}</div>
                        <div className="fs-12 text-muted">{r.customerPhone || '-'}</div>
                      </td>
                      <td>
                      ...
                      <td>
                        <select
                          className={`form-select form-select-sm ${STATUS_BADGE_CLASS[r.status]}`}
                          value={r.status}
